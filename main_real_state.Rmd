...
To validate the slope values obtained in the previous section, it is important to check if the regressor is subject to a Multicollinearity problem.
...
round(vif_lm_expl,1)%>%kbl(col.names=NULL)%>%kable_styling(bootstrap_options = c("striped", "hover", "condensed"),fixed_thead = T,full_width = F) %>%
  kable_material()
