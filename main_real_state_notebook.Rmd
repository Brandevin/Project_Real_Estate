...
* The variables Fireplaces and TotalBsmtSF, GarageArea, TotalRmsAbvGrd, X1stFlrSF were very correlated to other variables and will be eliminated for the final model.
...
To measure this phenomenom, the Variation Inflation Factor (VIF) is used. This factor is calculated as the variance of the predictor in the multi-regressor divided by the variance if this predictor was used alone in a regressor. A VIF equal to 1.7 means that the variance of this predictor's coefficient in the model is 70% higher than it would be if only this predictor was used in the model.
...
``` {r plot_skewness, width=8,height=6}
...
By employing a model with only numerical features and no logarithm correction, an RMSLE of **`r round(sc1*1000)/1000`** was obtained. On the other hand, using these same features but applying a logarithm transformation to the most skewed variables led to an RMSLE of **`r round(sc2*1000)/1000`**.

#### **3.2 Categorical variables**

##### **3.2.1 Transforming categorical variables into dummy variables**

In order for the models to process the categorical data, these were transformed into multiple binary variables, as seen in the table below. There are a total of **`r dim(parsed_matrix_train_common)[1]`** features.
...
head(data.frame(parsed_matrix_train_common),10)%>%kbl()%>%kable_styling(bootstrap_options = c("striped", "hover", "condensed"),fixed_thead = T,full_width = F) %>%
  kable_material()
...
##### **3.2.2 Recursive Feature elimination (RFE) and most important variables**

As there are a lot of features, training a model using all of them would lead the more complex models to overfit to the training data. To avoid this, a recursive feature elimination (RFE) was performed.
...
In the plot below, the RMSE for different numbers of selected variables is shown. It is possible to see that, using more than 60 features does not increase the model accuracy.

```{r feat_elimination,eval=FALSE}
...
```
 ...
 As such, only the top 60 features will be used. Their importance, calculated by considering how essential they were in a random forest model (how many times they appeared and how close they were to the top of the tree), is shown below. It is possible to see the most important features were the continuous ones analyzed in the previous section.
 
 Also important was the type of dwelling (MSSubClass), the classification of the sale (MSZoning) and the Neighbourhood.



```{r feat_elimination2, fig.width=10,fig.height=13,eval=FALSE}
...
rfe_selected_vars <- result_rfe2$variables %>% 
...
ggplot(data = varimp_data_top_values_df, 
...
Multiple models were trained using the data, each model had its hyperparameters optimized to minimize the RMSLE between the prediction and the training data.

``` {r testing_models,results='asis',WARNING=F}
...
parsed_matrix_train_common_top_65=parsed_matrix_train_common[,intersect(feat_imp_order,colnames(parsed_matrix_test_common))]
parsed_matrix_test_common_top_65=parsed_matrix_test_common[,intersect(feat_imp_order,colnames(parsed_matrix_test_common))]
...
tuneResult <- tune(svm,  train.x=data.frame(parsed_matrix_train_common_top_65),train.y=y_train,
              ranges = list(epsilon = seq(0.1,0.16,0.02), cost = 2^(0:4),gamma=2^(-8:-4))
)
...
c(sc_lm,sc_xgb,sc_svm,sc_avg)
```



A linear model was trained using the original variables, before the creation of the dummy variables. It obtained an RMSLE equal to **`r round(1000*sc_lm)/1000`**.

A X-Gradient Boosting model was trained using the 65 selected features. It obtained an RMSLE equal to **`r round(1000*sc_xgb)/1000`**. The best performing parameters were:

A support vector machine model was trained using the 65 selected features. It obtained an RMSLE equal to **`r round(1000*sc_svm)/1000`**. The best performing parameters were:

All of the model results were averaged with equal weight to obtain a new prediction. This prediction had an RMSLE equal to **`r round(1000*sc_avg)/1000`**.
